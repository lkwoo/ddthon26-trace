# PROJECT EVALUATION CRITERIA (ALWAYS IN EFFECT)
# All work in this project is centered on the assessment criteria below.
# This is auto-loaded every session — it survives `/clear` and new sessions.
@requirments/assessment.md

# MANDATORY: Auto-Record Development Process in Git (ALWAYS IN EFFECT)
# 이 프로젝트는 "AI-DLC 사용 충실성" 평가를 위해, 개발 과정 전체가 git 이력에
# 자동으로 남도록 한다. 단계별 결정이 뒤 단계로 이어졌다는 증거를 git 로그가 그대로
# 보여주는 것이 목적이다.

**핵심 원칙**: 의미 있는 산출물이 만들어지거나 갱신될 때마다 즉시 git 커밋을 남긴다.
누가 시키지 않아도 아래 시점에 스스로 커밋한다.

**자동 커밋 시점 (each는 승인 직후, 같은 interaction 안에서 수행)**:
- 각 AI-DLC 단계(stage)가 사용자 승인을 받아 완료될 때 (요구사항, 유저스토리, 워크플로우
  계획, 설계, 코드 생성 등)
- `aidlc-docs/` 산출물 문서가 생성/수정될 때
- `aidlc-docs/aidlc-state.md` 의 단계 진행 상태가 갱신될 때
- 애플리케이션 코드가 실제로 생성/변경될 때
- 빌드·테스트 지침이나 결과가 추가될 때

**커밋 메시지 규칙 (한국어, 어떤 과정이었는지 드러나게)**:
- 형식: `[AI-DLC:<Phase>/<Stage>] <무엇을 왜> — <앞 단계의 어떤 결정이 반영됐는지>`
- 예: `[AI-DLC:Inception/Requirements] TRACE 요구사항 확정 — intent 분석에서 도출한 MCP 단독 방향 반영`
- 예: `[AI-DLC:Construction/CodeGeneration] MCP 서버 tool 핸들러 구현 — functional-design의 데이터 모델 반영`
- 본문(선택)에는 이 단계에서 내린 결정과 다음 단계로 넘긴 컨텍스트를 1~3줄로 적는다.
- 커밋 메시지 말미에 다음 트레일러를 붙인다:
  `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`

**절차**:
1. 단계 완료/문서 변경 후, `git add`로 관련 파일(aidlc-docs 및/또는 코드)을 스테이징한다.
2. 위 형식에 맞춰 커밋한다. (사용자 승인 없이 커밋 가능 — 단, push는 사용자가 요청할 때만)
3. 커밋한 사실과 커밋 메시지를 사용자에게 간단히 보고한다.

**주의**:
- 아직 승인되지 않았거나 진행 중인 초안은 커밋하지 않는다 (완결된 단위만 기록).
- 시크릿/키가 포함된 파일은 절대 커밋하지 않는다.
- push는 자동으로 하지 않는다. 사용자가 명시적으로 요청할 때만 push한다.
- audit.md는 append만 하며, 커밋 대상에 자연스럽게 포함한다.
