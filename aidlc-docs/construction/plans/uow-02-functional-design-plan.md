...
## 계획 스텝 (체크박스)
- [x] S1. `FeatureCandidate`(+List) 모델 정의(id/title/description/related_sources/rationale)
- [x] S2. `identify_features(assets)` 알고리즘 — 카탈로그 입력 + FeatureCandidateList 스키마 + 자동 식별·상한
- [x] S3. LLM 입력 전략 — 자산 카탈로그(경로+유형+발췌 상한), content 없는 자산 제외 (Q3=A)
- [x] S4. `generate_feature_knowledge` — 지식 셸(body_markdown 개요, claims/evidence/conflicts=[]) (Q2=A)
- [x] S5. C4 저장소 — KnowledgeStore(save/load/list/read_resource), serialize/deserialize 사용
- [x] S6. 캐시 — compute_assets_hash(콘텐츠 해시)·AnalysisCache(.trace/cache) (Q4=A)
- [x] S7. .trace 루트=프로젝트 하위(Q5=A) + 결정성·부분실패(LLM warning 강등) 규칙
