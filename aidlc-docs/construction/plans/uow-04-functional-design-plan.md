...
## 계획 스텝 (체크박스)
- [ ] S1. 도메인 — TaskImpact LLM 중간 스키마(analyze_task step 출력) 설계 (기존 ImpactOut로 매핑)
- [ ] S2. 지식 컨텍스트 조립 — feature_id 유무별 대상 지식·근거 선별(그라운딩, FR-IMPACT-002)
- [ ] S3. 영향 분류 규칙 — Must/Likely/Review 판정·근거 참조·근거부족 강등(BR, NFR-AI-003)
- [ ] S4. 충돌 인지 경고(P1) — 대상 지식의 기존 conflicts를 related_conflicts로 노출(FR-IMPACT-004)
- [ ] S5. Change Plan — 순서형 단계 생성 규칙(충돌 해소 우선), 소스 자동수정 금지(FR-IMPACT-006)
- [ ] S6. analyze_task_impact 조립 — 컨텍스트→LLM→ImpactOut→build_result, 실패 강등(BR-PIPE)
- [ ] S7. 배치/알고리즘 — impact/ 모듈·시그니처·데이터 흐름·DoD·테스트 전략
- [ ] S8. 산출물 3종 작성(domain-entities/business-rules/business-logic-model)
