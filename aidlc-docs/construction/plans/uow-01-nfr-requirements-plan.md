...
## 계획 스텝 (체크박스)
- [x] S1. 성능/규모 NFR — Q1=A 단일 스레드 순차, Q2=B 파일당 5MB/총량 200MB 상한
- [x] S2. 신뢰성 NFR (Resiliency 확장) — 부분 실패/폴백 정책을 FR-ANALYSIS-003과 정합화
- [x] S3. 보안 NFR — NFR-SEC-004 + 심볼릭/트래버설/메시지 위생 확정
- [x] S4. 유지보수 NFR — 파서 확장성·상수/설정 분리(Q4=A config)
- [x] S5. PBT 속성 대상 확정 — Q3=A,B,C,D 전부(분류 전결정성/경로 안전성/부분실패 격리/결정성 멱등)
- [x] S6. nfr-requirements.md / tech-stack-decisions.md 작성 + 확장 컴플라이언스 요약
