...
## 계획 스텝 (체크박스)
- [x] S1. SDK/전송 — MCPServer 고수준 데코레이터·stdio, 의존성 버전 결정(Q1)
- [x] S2. 코어 래퍼 — list_features/get_feature_knowledge를 Result 반환 얇은 코어로 추가(engine)
- [x] S3. 도구 배선 — 5 도구=코어함수 1:1, 입력 스키마(타입힌트), 프로젝트 루트 주입(Q2)
- [x] S4. 직렬화/UX — Result→JSON, 핵심 우선 순서(요약→conflicts→impact→evidence, NFR-MCP-UX-002)
- [x] S5. 신뢰성 — 어댑터 오류 격리(코어 예외→구조화 오류 Result, 서버 무크래시)(Q3)
- [x] S6. 리소스/프롬프트 — trace://feature/<id> 리소스, (P1)구현 전 검토 프롬프트(Q4)
- [x] S7. 테스트 — 도구 등록/직렬화/오류매핑(오프라인), stdio 기동 스모크
- [x] S8. nfr-requirements.md / tech-stack-decisions.md 작성 + 확장 컴플라이언스 요약
