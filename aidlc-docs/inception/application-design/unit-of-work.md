...
- **범위 결정(2026-09-09)**: **3 페르소나 흐름 모두 재현** — P1 데브(변경 착수: analyze→list_features→analyze_task_impact),
  P2 마이라(충돌/장애 조사: get_conflicts 중심 + 낡은 지식), P3 피엠(요구사항 변경 파급: 자연어 task → impact/change_plan).
  데이터셋은 세 흐름 각각 최소 1개 대표 시나리오를 지원해야 함. (평가자용 사용 여정 문서의 데이터 전제)
...
- **평가자용 산출물(2026-09-09 결정)**: **페르소나별 사용 여정 문서**(`result/usage-walkthrough.md` 또는 README 섹션) —
  P1/P2/P3 각각 [언제·빈도(트리거) → 자연어 요청 → 호출 MCP 도구 → **실제 스크린샷** → 이점(TRACE 부재 대비)] 형식.
  각 여정 단계는 실제 동작하는 도구 호출과 진짜 스크린샷으로 뒷받침(완성도: 실행증거↔기능 정합).
  평가 배점 연결: 문제정의(20)·사용성(15)·완성도(15).
- **컴포넌트**: 횡단 + C9(CLI)
- **의존**: 전체(UOW-0F~05)
