...
- **실행 증거** (hero-demo-output.txt ①②):
  ```
  2개 Feature 분석 완료 (자산 13개, 충돌 3건)
    - owner-registration: Owner Registration (충돌 2건)
    - pet-management: Pet Management (충돌 1건)
  ```
  각 Feature 지식은 `trace://feature/<id>` 리소스(Markdown 본문)와 구조화 claims/confidence로 제공.
- **이점**: TRACE 없이는 여러 파일을 직접 읽어 머릿속에서 조립해야 함. TRACE는 **자산을 기능 단위로
  재구성 + 근거 참조**를 즉시 제공 → 온보딩 시간 단축, 추측 대신 근거.
...
- **실행 증거** (hero-demo-output.txt ③):
  ```
  - [value_mismatch]   owner.telephone.max_length : 20(spec.pdf) vs 10(openapi/code/db)
  - [policy_conflict]  owner.email.required       : required(spec.pdf) vs absent(openapi/code)
  - [stale_knowledge]  pet.birthdate.future_date_validation : rejected(notes) vs accepted(code)
  ```
  세 유형(값 불일치·정책 충돌·오래된 지식)이 **결정적 구조 비교**로 검출되고 각 값의 출처 파일이 붙음.
- **이점**: TRACE 없이는 "문서가 맞나 코드가 맞나"를 사람이 파일을 오가며 대조. TRACE는 **어긋난 지점과
  양쪽 근거**를 한눈에 → 조사 시간 단축, 놓친 불일치 방지.
